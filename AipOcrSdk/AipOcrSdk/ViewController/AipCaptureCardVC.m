    ...
    switch (self.cardType) {
        case CardTypeIdCardFont:{
            [[AipOcrService shardService] detectIdCardFrontFromImage:finalImage withOptions:nil successHandler:^(id result) {
                if ([self.delegate respondsToSelector:@selector(ocrOnIdCardSuccessful:)]) {
                    [self.delegate ocrOnIdCardSuccessful:result];
                }
...
//                [self showBankCardResultWithInfo:result[@"result"]];
                if ([self.delegate respondsToSelector:@selector(ocrOnBankCardSuccessful:)]) {
                    [self.delegate ocrOnBankCardSuccessful:result];
                }
            } failHandler:^(NSError *err) {
//                NSLog(@"%@", err);
                if ([self.delegate respondsToSelector:@selector(ocrOnFail:)]) {
