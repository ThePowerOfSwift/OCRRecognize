...
@interface PopoverViewEX () <UITableViewDelegate, UITableViewDataSource>
...
//    _keyWindow = [UIApplication sharedApplication].keyWindow;
    _windowWidth = CGRectGetWidth([UIScreen mainScreen].bounds);
    _windowHeight = CGRectGetHeight([UIScreen mainScreen].bounds);
...
- (void)showToPoint:(CGPoint)toPoint {
    ...
    // 限制最高高度, 免得选项太多时超出屏幕
    CGFloat maxHeight = _isUpward ? (_windowHeight - toPoint.y - kPopoverViewMargin) : (toPoint.y - 20);
...
#pragma GCC diagnostic pop
    ...
    if (maxWidth > CGRectGetWidth([UIScreen mainScreen].bounds) - kPopoverViewMargin*2) {
        maxWidth = CGRectGetWidth([UIScreen mainScreen].bounds) - kPopoverViewMargin*2;
    }
